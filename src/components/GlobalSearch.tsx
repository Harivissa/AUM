    ...
    <div className="fixed inset-0 z-[80] bg-black/75 backdrop-blur-xl p-4 sm:p-8" role="dialog" aria-modal="true" aria-label="Search AUM">
      <div className="max-w-3xl mx-auto mt-16 sm:mt-24 rounded-[2rem] border border-gold-400/25 bg-[#070611]/95 shadow-[0_30px_120px_rgba(0,0,0,.75)] overflow-hidden">
        <div className="p-5 border-b border-gold-500/15 flex items-center gap-3">
          ...
          <input autoFocus value={query} onChange={(e) => setQuery(e.target.value)} placeholder={t('Search AUM knowledge...')} className="flex-1 bg-transparent outline-none text-gold-100 placeholder-gold-500/40 font-body text-sm" />
        ...
        <div className="max-h-[62vh] overflow-y-auto p-3">
          {results.length ? results.map(([title, description, href], idx) => (
            <button key={`search-${href}-${title}-${idx}`} onClick={() => go(href)} className="w-full text-left flex items-center justify-between gap-4 p-4 rounded-2xl hover:bg-gold-500/10 border border-transparent hover:border-gold-500/15 transition min-w-0">
              <div className="min-w-0 flex-1"><div className="font-display text-lg text-gold-100 break-words">{title}</div><div className="mt-1 text-xs font-body leading-relaxed text-gold-300/60 break-words">{description}</div></div>
              <ArrowRight className="w-4 h-4 text-gold-400 shrink-0" />
            </button>
          )) : <div className="p-10 text-center text-sm font-body text-gold-300/60 break-words">{t('No AUM entry found for')} “{query}”.</div>}
        </div>
      </div>
    </div>
  )
