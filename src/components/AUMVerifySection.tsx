...
export default function AUMVerifySection() {
  ...
  const getVerdictBadge = (verdict: VerifiedClaim['verdict']) => {
    switch (verdict) {
      case 'Verified':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-950/70 border border-emerald-500/40 text-emerald-300 font-body text-xs font-semibold shadow-sm">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
            Scripturally Verified
          </span>
        )
      case 'Contextual Truth':
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-amber-950/70 border border-amber-500/40 text-amber-300 font-body text-xs font-semibold shadow-sm">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-950/70 border border-rose-500/40 text-rose-300 font-body text-xs font-semibold shadow-sm">
            <HelpCircle className="w-3.5 h-3.5 text-rose-400" />
            Source Not Found / Unverified
          </span>
        )
    }
  }
    ...
    <section id="verify" className="relative py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto z-10">
      ...
      <div className="text-center max-w-3xl mx-auto mb-14">
        ...
        <p className="mt-4 font-body text-sm sm:text-base text-gold-200/70 leading-relaxed font-light">
          Cut through viral internet myths, mistranslations, and sensationalism. Cross-reference quotes,
          philosophical concepts, and historical facts directly against authentic Sanskrit critical editions.
        </p>
        ...
        <form onSubmit={handleCustomVerify} className="mt-8 max-w-2xl mx-auto flex items-center gap-2 relative">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Ask or verify a claim (e.g., Speed of light in Rigveda, Meaning of Maya...)"
          ...
          <button
            ...
            className="px-6 py-3.5 rounded-full bg-gradient-to-r from-gold-500 to-gold-400 text-void font-body font-semibold text-xs sm:text-sm shadow-[0_0_20px_rgba(232,197,107,0.3)] hover:brightness-110 transition shrink-0"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-5 flex flex-col gap-3">
          <span className="font-body text-xs uppercase tracking-widest2 text-gold-400 font-semibold px-2 mb-1">
            Verified Source Records ({filteredClaims.length})
          </span>
            ...
            return (
              <TiltCard3D key={claim.id} intensity={8} glare={false}>
                <button
                  ...
                  className={`w-full text-left p-4 sm:p-5 rounded-2xl transition-all duration-200 border flex flex-col justify-between ${
                    isSelected
                ...
                >
                  <div className="flex items-center justify-between w-full mb-2">
                    <span className="font-body text-[11px] text-gold-400 font-semibold">{claim.category}</span>
                    {getVerdictBadge(claim.verdict)}
                  </div>
                  <h3 className="font-display text-base font-bold text-gold-100 leading-snug">{claim.query}</h3>
                  <p className="font-body text-xs text-gold-300/60 mt-1.5 line-clamp-2">{claim.summary}</p>
                </button>
              </TiltCard3D>
            )
          })}
        ...
        <div className="lg:col-span-7">
          <TiltCard3D intensity={10} className="w-full">
            <div className="rounded-3xl bg-gradient-to-b from-[#0e0a27]/95 via-[#070514] to-[#04030a] border border-gold-400/40 p-6 sm:p-8 shadow-2xl relative">
              {/* Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-gold-500/20 pb-5">
                ...
                <div>{getVerdictBadge(displayClaim.verdict)}</div>
              </div>

              {/* Summary Box */}
              <div className="mt-6 p-4 rounded-xl bg-gold-500/10 border border-gold-500/25">
              ...
              <div className="mt-6 p-5 rounded-2xl bg-black/60 border border-gold-500/20">
                <span className="font-body text-[11px] uppercase tracking-wider text-gold-500 font-semibold block mb-2 flex items-center gap-1.5">
                  ...
                  Primary Sanskrit Source Reference
                </span>
                <p className="font-deva text-base sm:text-lg text-gold-200 font-medium tracking-devanagari">
                  {displayClaim.sanskritSource.text}
                </p>
                <p className="font-body text-xs text-gold-400/80 italic mt-1.5">
                  {displayClaim.sanskritSource.transliteration}
                </p>
                <p className="font-body text-xs sm:text-sm text-gold-100/90 mt-2.5 pt-2.5 border-t border-gold-500/15">
                  "{displayClaim.englishTranslation}"
                </p>
                <span className="inline-block mt-2 font-body text-[11px] text-gold-400 font-semibold">
              ...
              {/* Scholarly Consensus */}
              <div className="mt-6 pt-4 border-t border-gold-500/15 flex items-center justify-between text-xs font-body text-gold-500/70">
                <span>Scholarly Citation: {displayClaim.scholarlyReference}</span>
              </div>
            </div>
          </TiltCard3D>
        </div>
