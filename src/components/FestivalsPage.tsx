...
function SourceBadge({ category }: { category: SourceCategory }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 font-body text-[9px] uppercase tracking-[.12em] ${styles[category]}`}>
      {category}
    </span>
  )
}
    ...
    <div className="relative overflow-hidden rounded-3xl border border-gold-500/25 bg-black/60 shadow-xl group">
      {!loaded && !error && (
        <div className="h-64 sm:h-80 flex flex-col items-center justify-center gap-2 text-gold-400/60 bg-gold-950/30">
          ...
          <span className="font-body text-[10px] uppercase tracking-wider">Loading high-resolution asset</span>
        </div>
      )}
      {!error ? (
        <img
...
export default function FestivalsPage({ onBack, onNavigate }: { onBack: () => void; onNavigate: (href: string) => void }) {
  ...
  const [selectedIconItem, setSelectedIconItem] = useState(festival.iconography[0])
  ...
  return (
    <main className="relative z-10 min-h-screen pt-28 pb-24 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <header className="relative mt-6 grid lg:grid-cols-[1.1fr_.9fr] gap-8 items-center rounded-[2.5rem] border border-gold-500/25 bg-gradient-to-b from-[#140c2b]/90 via-[#0a0718]/90 to-black/80 p-6 sm:p-10 shadow-2xl">
          <div>
            ...
            <p className="mt-2 font-body text-[10px] uppercase tracking-[.24em] text-gold-400 font-semibold">
              Living Sanātana Observance
            </p>
            <h1 className="mt-2 font-display text-5xl sm:text-7xl font-bold text-gold-100 text-glow">
              {festival.name}
            </h1>
            <p className="mt-2 font-display text-2xl italic text-gold-300">{festival.alternateName}</p>
            <p className="mt-5 max-w-2xl font-body text-sm sm:text-base leading-relaxed text-gold-200/75">
              {festival.summary}
            </p>
            <div className="mt-6 p-4 rounded-2xl bg-gold-950/40 border-l-4 border-gold-400 font-display text-lg italic text-gold-200/90 leading-relaxed">
              "{festival.meaningToday}"
            </div>
          </div>

          <LocalImageFrame
            src="media/ganesha_main.jpg"
            alt="Lord Ganesha Murti in traditional worship setting"
            caption="Śrī Gaṇeśa — Invoked at All Beginnings"
            sourceInfo="Source: Wikimedia Commons · CC BY-SA 4.0 · High-resolution media asset"
          />
        </header>

        {/* What is Vināyaka Chavithi & Calendar Context */}
        <section className="grid lg:grid-cols-3 gap-5 mt-10">
          <article className="lg:col-span-2 rounded-3xl border border-gold-500/20 bg-black/45 p-6 sm:p-8 shadow-lg">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h2 className="font-display text-2xl font-bold text-gold-100">What is Vināyaka Chavithi?</h2>
            </div>
            <p className="font-body text-sm sm:text-base leading-relaxed text-gold-200/75">
              Also known as <span className="text-gold-300 font-semibold">Gaṇeśa Caturthī</span>, it is an auspicious observance celebrated across India and global Hindu communities. It falls on the fourth lunar day (caturthī) of the bright fortnight (śukla pakṣa) in the month of Bhādrapada.
            </p>
            <p className="mt-3 font-body text-xs sm:text-sm leading-relaxed text-gold-300/70">
              {festival.observance} Because lunisolar calendars and local sunrise calculations vary by region, there is no single compulsory civil date or uniform ritual manual for every household.
            </p>
          </article>
          ...
          <article className="rounded-3xl border border-gold-500/20 bg-gold-950/30 p-6 sm:p-8 flex flex-col justify-between">
            <div>
              ...
              <h3 className="font-display text-xl font-bold text-gold-100">Regional Calendar Precision</h3>
              <p className="mt-2.5 font-body text-xs leading-relaxed text-gold-200/70">
                {festival.regionNote}
              </p>
            </div>
            <div className="mt-6 pt-4 border-t border-gold-500/15 font-body text-[10px] uppercase tracking-wider text-gold-400 font-semibold">
              AUM Source Policy: No Prescriptive Universal Formula
            </div>
          </article>
        </section>

        {/* Why is it celebrated? */}
        <section className="mt-14 rounded-3xl border border-gold-500/20 bg-gradient-to-r from-gold-950/40 via-black/50 to-gold-950/40 p-6 sm:p-8">
          <div className="max-w-4xl">
            <p className="font-body text-[10px] uppercase tracking-widest text-gold-400 font-semibold">Devotional & Philosophical Context</p>
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-gold-100 mt-1">Why is it Celebrated?</h2>
            <p className="mt-3 font-body text-sm sm:text-base leading-relaxed text-gold-200/80">
              Bhagavān Gaṇeśa is revered as <span className="text-gold-300 font-semibold">Vighneśvara</span> (Remover of Obstacles), <span className="text-gold-300 font-semibold">Buddhipradāyaka</span> (Bestower of Intellect), and the lord of auspicious beginnings. Devotees invoke his presence for wisdom, spiritual growth, discernment, and community harmony before undertaking any major venture.
            </p>
            <div className="mt-6 grid sm:grid-cols-3 gap-4">
              <div className="p-4 rounded-2xl bg-black/40 border border-gold-500/15">
                <span className="font-deva text-gold-400 text-sm">विघ्ननाशक</span>
                <h3 className="font-display text-base text-gold-100 font-semibold mt-1">Obstacle Removal</h3>
                <p className="font-body text-xs text-gold-300/65 mt-1">Clearing spiritual & mental barriers.</p>
              </div>
              <div className="p-4 rounded-2xl bg-black/40 border border-gold-500/15">
                <span className="font-deva text-gold-400 text-sm">बुद्धिदाता</span>
                <h3 className="font-display text-base text-gold-100 font-semibold mt-1">Wisdom & Learning</h3>
                <p className="font-body text-xs text-gold-300/65 mt-1">Discernment (Viveka) before action.</p>
              </div>
              <div className="p-4 rounded-2xl bg-black/40 border border-gold-500/15">
                <span className="font-deva text-gold-400 text-sm">प्रथमपूज्य</span>
                <h3 className="font-display text-base text-gold-100 font-semibold mt-1">First Invoked</h3>
                <p className="font-body text-xs text-gold-300/65 mt-1">Honoured first in all sacred rites.</p>
              </div>
            </div>
          </div>
        </section>

        {/* Why Gaṇeśa? Interactive Iconography Section */}
        <section className="mt-16">
          <div className="text-center max-w-3xl mx-auto mb-10">
            <span className="font-body text-[10px] uppercase tracking-widest text-gold-400 font-semibold">Sacred Symbolism</span>
            <h2 className="mt-1 font-display text-3xl sm:text-5xl font-bold text-gold-100 text-glow">
              Why Gaṇeśa? — Interactive Iconography
            </h2>
            <p className="mt-3 font-body text-sm text-gold-300/70 leading-relaxed">
              Every detail of Śrī Gaṇeśa's form carries profound traditional symbolism. Select any feature below to explore its symbolic meaning.
            </p>
          </div>
        ...
        <section className="mt-16">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
            <div>
              ...
              <h2 className="font-display text-3xl sm:text-4xl font-bold text-gold-100 mt-1">9-Step Festival Timeline</h2>
            </div>
            <p className="font-body text-xs text-gold-400/70 max-w-md">
              Duration varies by household and community (1.5, 3, 5, 7, or 11 days).
            </p>
          </div>
        ...
        {/* Why is Gaṇeśa Immersed in Water? Major Visual Section */}
        <section className="mt-16 rounded-3xl border border-gold-400/35 bg-gradient-to-b from-[#120a28]/95 via-[#080514] to-black p-6 sm:p-10 shadow-2xl">
          <div className="grid lg:grid-cols-[1.1fr_.9fr] gap-8 items-center">
            <div>
              ...
              <h2 className="font-display text-3xl sm:text-5xl font-bold text-gold-100 text-glow">
                Why is Gaṇeśa Immersed in Water?
              </h2>
              ...
              <div className="mt-6 space-y-3 font-body text-xs sm:text-sm text-gold-300/80">
                ...
                <div className="flex items-start gap-2.5">
                  <span className="text-gold-400 font-bold">✦</span>
                  <span><strong className="text-gold-100">Flexible Family Timelines:</strong> Immersions occur on the 1st, 1.5, 3rd, 5th, 7th, or 11th day per family tradition (Kula-dharma).</span>
                </div>
              </div>
            </div>
            ...
            <LocalImageFrame
              ...
              alt="Visarjana procession of Lord Ganesha idol to water"
              caption="Form → Worship → Gratitude → Dissolution → Renewal"
              sourceInfo="Source: Wikimedia Commons · CC BY-SA 3.0 · Visarjana ritual in water body"
            />
          </div>
        ...
        <section className="mt-16 grid lg:grid-cols-2 gap-8 items-center">
          <LocalImageFrame
            src="media/clay_murti.jpg"
            alt="Traditional unbaked natural clay Ganesha murtis"
            caption="Natural Clay (Shaddu Mati) Murtis"
            sourceInfo="Source: Wikimedia Commons · Handcrafted natural clay murtis without synthetic dyes"
          />

          <div className="space-y-4">
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-emerald-950/40 border border-emerald-500/30 text-emerald-300 font-body text-xs uppercase tracking-widest font-semibold">
              <Leaf className="w-3.5 h-3.5 text-emerald-400" /> Ecology & Dharmic Responsibility
            </div>
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-gold-100">
              Natural Earth & Ecological Duty
            </h2>
            <p className="font-body text-sm sm:text-base leading-relaxed text-gold-200/75">
              Traditional scriptures emphasize using unbaked natural clay (<span className="text-gold-300 italic">Mṛttikā</span>) and natural plant dyes. Modern synthetic Plaster of Paris (PoP) and chemical colors harm aquatic life and violate the core spirit of environmental care (<span className="text-gold-300 italic">Bhūmi Rakṣā</span>).
            </p>

            <div className="grid sm:grid-cols-2 gap-3 pt-2">
              <div className="p-4 rounded-2xl bg-black/45 border border-gold-500/15">
                <h3 className="font-display text-base font-bold text-gold-100">Home Immersion</h3>
                <p className="mt-1 font-body text-xs text-gold-300/65 leading-relaxed">
                  Immerse unpainted clay murti in a home vessel or bucket. Use dissolved earth for garden plants.
                </p>
              </div>
              <div className="p-4 rounded-2xl bg-black/45 border border-gold-500/15">
                <h3 className="font-display text-base font-bold text-gold-100">Nirmālya Recycling</h3>
                <p className="mt-1 font-body text-xs text-gold-300/65 leading-relaxed">
                  Collect offered flowers separately for organic composting rather than tossing plastic into water.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="mt-16">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-gold-100">
              Traditional Offerings (Naivedya)
            </h2>
          </div>
        ...
        <section className="mt-16">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {festival.regionalCards.map((card) => (
              <div key={card.region} className="p-6 rounded-3xl border border-gold-500/20 bg-black/45 gold-glow-box-hover transition-all">
                <span className="font-body text-[10px] uppercase tracking-wider text-gold-400 font-semibold">{card.region}</span>
                <h3 className="mt-2 font-display text-xl font-bold text-gold-100">{card.highlights}</h3>
        ...
        <section className="mt-16 border-t border-gold-500/20 pt-12">
          <div className="flex items-center gap-3 mb-6">
            <BookOpen className="w-5 h-5 text-gold-400" />
            <div>
              <h2 className="font-display text-3xl font-bold text-gold-100">Source Lens & Evidence Standard</h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {festival.sources.map((source) => (
              <div key={source.category} className="p-5 rounded-2xl border border-gold-500/15 bg-black/40">
